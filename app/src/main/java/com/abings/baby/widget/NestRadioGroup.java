...
/**
 * 支持嵌套CompoundButton的NestRadioGroup new_message_tips可以是一个类似微信右上角的小红圈，用来提醒有新的消息。
 * 仿RadioGroup做的，所以用法跟RadioGroup一样
 */
...
public class NestRadioGroup extends LinearLayout {
	...
	private class PassThroughHierarchyChangeListener implements
		...
		/**
		 * {@inheritDoc}View.generateViewId需要4.2以上才能使用，所以最好自己设置id
		 */
		...
		public void onChildViewAdded(View parent, View child) {
			if (parent == NestRadioGroup.this) {
				...
				if (view != null) {
					...
					/*
					 * if (id == View.NO_ID && Build.VERSION.SDK_INT >=
					 * Build.VERSION_CODES.JELLY_BEAN_MR1) { id =
					 * View.generateViewId(); view.setId(id); }
					 */
					if (id == View.NO_ID) {
						id = NestRadioGroup.this.getChildCount() + 1;
						view.setId(id);
					}
