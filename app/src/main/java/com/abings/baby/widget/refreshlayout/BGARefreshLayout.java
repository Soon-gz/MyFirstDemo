...
public class BGARefreshLayout extends LinearLayout {
    ...
    public void startChangeWholeHeaderViewPaddingTop(int distance) {
        ValueAnimator animator = ValueAnimator.ofInt(mWholeHeaderView.getPaddingTop(), mWholeHeaderView.getPaddingTop
                () - distance);
            ...
            mRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
                ...
                public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                    if ((newState == RecyclerView.SCROLL_STATE_IDLE || newState == RecyclerView
                            .SCROLL_STATE_SETTLING) && shouldHandleRecyclerViewLoadingMore(mRecyclerView)) {
                        beginLoadingMore();
                    }
                }
            });
    ...
    private void setAbsListViewOnScrollListener() {
        if (mAbsListView != null) {
            try {
                ...
                // mAbsListView.getClass().getDeclaredField("mOnScrollListener")获取不到mOnScrollListener，必须通过AbsListView
                // .class.getDeclaredField("mOnScrollListener")获取
                Field field = AbsListView.class.getDeclaredField("mOnScrollListener");
                field.setAccessible(true);
                // 开发者自定义的滚动监听器
                final AbsListView.OnScrollListener onScrollListener = (AbsListView.OnScrollListener) field.get
                        (mAbsListView);
    ...
    public boolean shouldHandleAbsListViewLoadingMore(AbsListView absListView) {
        if (mIsLoadingMore || mCurrentRefreshStatus == RefreshStatus.REFRESHING || mLoadMoreFooterView == null ||
                mDelegate == null || absListView == null || absListView.getAdapter() == null || absListView
    ...
    public boolean shouldHandleRecyclerViewLoadingMore(RecyclerView recyclerView) {
        if (mIsLoadingMore || mCurrentRefreshStatus == RefreshStatus.REFRESHING || mLoadMoreFooterView == null ||
                mDelegate == null || recyclerView.getAdapter() == null || recyclerView.getAdapter().getItemCount() ==
                0) {
            return false;
        }
    ...
    private boolean shouldHandleRefresh() {
        if (!mPullDownRefreshEnable || mIsLoadingMore || mCurrentRefreshStatus == RefreshStatus.REFRESHING ||
    ...
    private boolean handleActionMove(MotionEvent event) {
        ...
        if (mCustomHeaderView != null && mIsCustomHeaderViewScrollable) {
            ...
            if ((mPullDownRefreshEnable && !isWholeHeaderViewCompleteInvisible()) || (wholeHeaderDiffY > 0 &&
                    shouldInterceptToMoveCustomHeaderViewDown()) || (wholeHeaderDiffY < 0 &&
